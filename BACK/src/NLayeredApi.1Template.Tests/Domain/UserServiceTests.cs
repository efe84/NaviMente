...
namespace NaviMente.Tests.Domain
{
    public class UserServiceTests
    {
        ...
        [Fact]
        public void AddPhone_ShouldAddPhone_WhenUserExists()
        {
            // Arrange
            var username = "existinguser";
            var newPhone = "555-0000";
            var userBefore = new User { Username = username, OtherPhones = new List<string>() };
            var userAfter = new User { Username = username, OtherPhones = new List<string> { newPhone } };

            _userQueryRepository.SetupSequence(r => r.GetByUsername(username))
                .Returns(userBefore) // primera llamada: usuario existe
                .Returns(userAfter); // segunda llamada: usuario con nuevo teléfono

            // Act
            var result = _userService.AddPhone(username, newPhone);

            // Assert
            _userQueryRepository.Verify(r => r.AddPhone(username, newPhone), Times.Once);
            Assert.NotNull(result);
            Assert.Contains(newPhone, result.OtherPhones);
        }

        [Fact]
        public void AddPhone_ShouldThrowException_WhenUserDoesNotExist()
        {
            // Arrange
            var username = "nonexistent";
            var newPhone = "555-0000";
            ...
            // Act & Assert
            var ex = Assert.Throws<Exception>(() => _userService.AddPhone(username, newPhone));
            Assert.Equal($"User to edit not found {username}", ex.Message);

            _userQueryRepository.Verify(r => r.AddPhone(It.IsAny<string>(), It.IsAny<string>()), Times.Never);
        }

        [Fact]
        public void AddPhone_ShouldReturnNull_WhenUserNotFoundAfterAdd()
        {
            // Arrange
            var username = "existinguser";
            var newPhone = "555-0000";
            ...
            _userQueryRepository.SetupSequence(r => r.GetByUsername(username))
                .Returns(userBefore)  // primera llamada: existe
                .Returns((User)null); // segunda llamada: no encontrado después del AddPhone
        ...
        [Fact]
        public void RemovePhone_ShouldRemovePhone_WhenUserExists()
        {
            // Arrange
            var username = "existinguser";
            var phoneToRemove = "555-0000";
            var userBefore = new User { Username = username, OtherPhones = new List<string> { phoneToRemove, "123-4567" } };
            var userAfter = new User { Username = username, OtherPhones = new List<string> { "123-4567" } };

            _userQueryRepository.SetupSequence(r => r.GetByUsername(username))
                .Returns(userBefore) // primera llamada: usuario existe con el teléfono
                .Returns(userAfter); // segunda llamada: teléfono eliminado

            // Act
            var result = _userService.RemovePhone(username, phoneToRemove);

            // Assert
            _userQueryRepository.Verify(r => r.RemovePhone(username, phoneToRemove), Times.Once);
            Assert.NotNull(result);
            Assert.DoesNotContain(phoneToRemove, result.OtherPhones);
        }

        [Fact]
        public void RemovePhone_ShouldThrowException_WhenUserDoesNotExist()
        {
            // Arrange
            var username = "nonexistent";
            var phoneToRemove = "555-0000";
            ...
            // Act & Assert
            var ex = Assert.Throws<Exception>(() => _userService.RemovePhone(username, phoneToRemove));
            Assert.Equal($"User to edit not found {username}", ex.Message);

            _userQueryRepository.Verify(r => r.RemovePhone(It.IsAny<string>(), It.IsAny<string>()), Times.Never);
        }

        [Fact]
        public void RemovePhone_ShouldReturnNull_WhenUserNotFoundAfterRemove()
        {
            // Arrange
            var username = "existinguser";
            var phoneToRemove = "555-0000";
            var userBefore = new User { Username = username, OtherPhones = new List<string> { phoneToRemove } };

            _userQueryRepository.SetupSequence(r => r.GetByUsername(username))
                .Returns(userBefore)  // primera llamada: usuario existe
                .Returns((User)null); // segunda llamada: no encontrado tras remover teléfono
