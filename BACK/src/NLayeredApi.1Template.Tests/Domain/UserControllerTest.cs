...
namespace NaviMente.Tests.Domain
{
    public class UserControllerTests
    {
        ...
        [Fact]
        public void AddPhone_ReturnsOk_WhenSuccess()
        {
            string username = "user1";
            var newPhoneDto = new NewPhoneDTO { NewPhone = "987654321" };
            var updatedUser = new User { Username = username };

            _userServiceMock.Setup(s => s.AddPhone(username, newPhoneDto.NewPhone)).Returns(updatedUser);

            var result = _userController.AddPhone(username, newPhoneDto);
        ...
        [Fact]
        public void AddPhone_ReturnsBadRequest_WhenUserNull()
        {
            string username = "user1";
            var newPhoneDto = new NewPhoneDTO { NewPhone = "987654321" };

            _userServiceMock.Setup(s => s.AddPhone(username, newPhoneDto.NewPhone)).Returns((User?)null);

            var result = _userController.AddPhone(username, newPhoneDto);

            var badRequestResult = Assert.IsType<BadRequestObjectResult>(result);
            Assert.Equal("Error añadiendo teléfono al usuario", badRequestResult.Value);
        }

        [Fact]
        public void AddPhone_ReturnsBadRequest_WhenExceptionThrown()
        {
            string username = "user1";
            var newPhoneDto = new NewPhoneDTO { NewPhone = "987654321" };

            _userServiceMock.Setup(s => s.AddPhone(username, newPhoneDto.NewPhone))
                            .Throws(new Exception("error"));
        ...
        [Fact]
        public void DeletePhone_ReturnsOk_WhenSuccess()
        {
            string username = "user1";
            string phoneNumber = "987654321";
            var updatedUser = new User { Username = username };

            _userServiceMock.Setup(s => s.RemovePhone(username, phoneNumber)).Returns(updatedUser);

            var result = _userController.DeletePhone(username, phoneNumber);
        ...
        [Fact]
        public void DeletePhone_ReturnsBadRequest_WhenUserNull()
        {
            string username = "user1";
            string phoneNumber = "987654321";

            _userServiceMock.Setup(s => s.RemovePhone(username, phoneNumber)).Returns((User?)null);

            var result = _userController.DeletePhone(username, phoneNumber);

            var badRequestResult = Assert.IsType<BadRequestObjectResult>(result);
            Assert.Equal("Error eliminando teléfono al usuario", badRequestResult.Value);
        }

        [Fact]
        public void DeletePhone_ReturnsBadRequest_WhenExceptionThrown()
        {
            string username = "user1";
            string phoneNumber = "987654321";

            _userServiceMock.Setup(s => s.RemovePhone(username, phoneNumber))
                            .Throws(new Exception("error"));
